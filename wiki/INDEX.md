...
| [specs/B39-order-cancel-regression.md](specs/B39-order-cancel-regression.md) | Spec: `/order/cancel/` regression — 8 in-scope findings across PageHero (`eyebrowContent` snippet, `headingVariant: 'hero' \| 'title'` prop default `'title'`, vertical rhythm), Heading (new `title` variant; `hero` letter-spacing -0.02em), Rule + Button `back` defensive locks, Text eyebrow 12→14 px + lede tracking/line-height. Signoff pattern (finding 9) dropped — kept local to website. Mixed src-only + behavioural ACs per D42. |
