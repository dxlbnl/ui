...
<Story name="Ok"
  args={{ id: 'toast-1', message: 'Build completed successfully.', variant: 'ok', ondismiss: () => {} }}
  play={async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText('Build completed successfully.')).toBeVisible();
...
<Story name="Amber"
  args={{ id: 'toast-2', message: '+12V rail at 88% capacity.', variant: 'amber', ondismiss: () => {} }}
  play={async ({ canvasElement }) => {
    const canvas = within(canvasElement);
...
<Story name="Danger"
  args={{ id: 'toast-3', message: 'Thermal protection triggered.', variant: 'danger', ondismiss: () => {} }}
  play={async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    ...
    // AC 26: message text visible
    await expect(canvas.getByText('Thermal protection triggered.')).toBeVisible();
...
<Story name="Manual Close"
  args={{ id: 'toast-4', message: 'Click × to dismiss.', variant: 'ok', ondismiss: () => {} }}
...
<Story name="Long Message"
  args={{ id: 'toast-5', message: 'This is a longer notification message that tests how the toast handles wrapping text within its maximum width constraint.', variant: 'amber', ondismiss: () => {} }}
  play={async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    ...
    await expect(canvas.getByText('This is a longer notification message that tests how the toast handles wrapping text within its maximum width constraint.')).toBeVisible();
...
<Story name="Aria Attributes"
  args={{ id: 'toast-6', message: 'ARIA check.', variant: 'ok', ondismiss: () => {} }}
  play={async ({ canvasElement }) => {
    const canvas = within(canvasElement);
